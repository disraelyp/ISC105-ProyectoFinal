...
int juego(jugador *a, jugador *b, tablero *contenedor){
    int ciclo=0, cont=0, op;
    char inicio[2], final[2];
    do{
...
int cambio_posicion(tablero *cont, jugador *a, posiciones inicial, posiciones final){
    ...
    (*(*(contenedor + yFinal) + xFinal))=creacion_bloque(a, calcular_cordenada(final));
    (*(*(contenedor + yInicial) + xInicial))=creacion_bloque(NULL, calcular_cordenada(inicial));
    ...
    if (verificar_frase(movimiento)){
        ...
        if(posiciones_jugables(inicial) && posiciones_jugables(final)){
            if(verificar_movimiento(*(cont), inicial, final)){
                ...
                printf("\n\tERORR: LA POSICION FINAL YA ESTA OCUPADA POR OTRO PEON, O LA INICIAL ESTA VACIA.\n\n");
            }
        } else {
