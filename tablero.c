...
int verificar_movimiento(tablero cont, posiciones pieza){
    bloque **contenedor=cont.plano;
    int x=calcular_cordenadaX(pieza), y=calcular_cordenadaY(pieza);
    if((*(*(contenedor+y)+x)).peones.propietario->representacion==blanco && (*(*(contenedor+y)+x)).estado!=casilla_blanca){
        if ((*(*(contenedor+(y+1))+(x-1))).estado==disponible && x>=0 && y<=7){
            if ((*(*(contenedor+(y+1))+(x+1))).estado==disponible && x<=7 && y<=7){
                return 2;
            }
            return 1;
        }
    ...
    if((*(*(contenedor+y)+x)).peones.propietario->representacion==negro && (*(*(contenedor+y)+x)).estado!=casilla_blanca){
        if ((*(*(contenedor+(y-1))+(x-1))).estado==disponible && x>=0 && y<=7){
            if ((*(*(contenedor+(y-1))+(x+1))).estado==disponible && x<=7 && y>=0){
                return 2;
            }
            return 1;
        }
